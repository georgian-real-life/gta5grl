...
user.getDepartmentName = function(player) {
    try {
        ...
        if (user.get(player, 'is_leader'))
            ...
            return 'Руководство';
        return enums.fractionListId[user.get(player, 'fraction_id')].departmentList[user.get(player, 'rank_type1')];
    }
    catch (e) {
        methods.debug(e);
    }
...
user.getRankName = function(player) {
    try {
        ...
        return enums.fractionListId[user.get(player, 'fraction_id')].rankList[user.get(player, 'rank_type1')][user.get(player, 'rank1')];
    }
    catch (e) {
        methods.debug(e);
    }
    ...
    if (user.get(player, 'bank_card') > 0) {
        ...
        if (player.getVariable('isAfk') === true) {
            ...
            if (!user.hasById(user.getId(player), 'uniform') && !user.isNews(player) && !user.isFib(player))
            {
                ...
                let money = methods.getFractionPayDay(user.get(player, 'fraction_id'), user.get(player, 'rank1'), user.get(player, 'rank_type1'));
...
user.isDepLeader = function(player) {
    ...
    return user.isLogin(player) && user.get(player, 'fraction_id') > 0 && user.get(player, 'rank1') === 0;
};

user.isDepSubLeader = function(player) {
    //methods.debug('user.isDepSubLeader');
    return user.isLogin(player) && user.get(player, 'fraction_id') > 0 && user.get(player, 'rank1') === 1;
};
...
user.giveUniform = function(player, id = 0) {
    ...
    if (id === 0) { //default
        ...
        if (user.getSex(player) == 1) {
            ...
            if (user.get(player, 'rank1') === 2 && user.get(player, 'rank_type1') > 0)
                user.setComponentVariation(player, 10, 7, 1);
            else if (user.get(player, 'rank1') === 1)
                user.setComponentVariation(player, 10, 7, 2);
            else if (user.get(player, 'rank1') === 0)
