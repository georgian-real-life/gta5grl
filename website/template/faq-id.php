...
<div style="width: 100%; overflow: hidden; position: absolute; height: 100%">
    <img src="<?php
error_reporting(0); echo $this->faq['img'] ?>" style="filter: blur(10px); z-index: -1; position: absolute; top: 0; left: -20px; width: 110%; max-height: 450px;">
</div>
    ...
    <div class="section">
        <div class="row">
            ...
            <div class="col s12 m10 l8">
                <?php
error_reporting(0); echo ($userInfo['admin_level'] > 0) ? '<a href="#editFaq" class="modal-trigger btn blue accent-4 waves-effect">Редактировать</a>' : '' ?>
                <div class="center-block card-panel">
                    <h4 class="wd-font"><?php
error_reporting(0); echo $this->faq['title'] ?></h4>
                    <?php
error_reporting(0); echo str_replace('<img src', '<img style="margin: 0 -24px;width: calc(100% + 48px);" class="materialboxed" src', nl2br(htmlspecialchars_decode(htmlspecialchars_decode($this->faq['text'])))) ?>
                </div>
            </div>
            <div class="col s12 m1 l2">
            </div>
