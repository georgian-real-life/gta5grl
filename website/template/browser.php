...
<head>
    ...
    <script type="text/javascript">
        $document.ready(function() {
            let currentUrl = 'https://test1.ru/network/search';

            let history = [['Главная', 'https://test1.ru/network/search']];
            ...
            $('#iframeid').load(function() {
                ...
                $('#search').val(currentUrl.replace('https://test1.ru/network', 'network.sa'));
            ...
            $('#search').keyup(function(e){
                ...
                {
                    let val = $('#search').val();
                    if (val.indexOf('test1.ru') >= 0)
                        $('#iframeid').attr('src', val);
                    else if (val.indexOf('network.sa') >= 0)
                        $('#iframeid').attr('src', val.replace('network.sa', 'https://test1.ru/network'));
                    else
                        $('#iframeid').attr('src', 'https://test1.ru/network/search?q=' + val);
                }
            });
        });
