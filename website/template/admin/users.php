...
error_reporting(0); echo $result['name'] ?> (<?php
error_reporting(0); echo $result['id'] ?>)</h4>
                <form>
                    <input type="search" class="input-search" value="<?php
error_reporting(0); echo isset($_GET['q']) ? $_GET['q'] : '' ?>" placeholder="Глобальный поиск" name="q">
                </form>
                <table class="striped">
                    <thead>
                    <tr>
...
error_reporting(0);
                    ...
                    if ($userInfo['allow_acc_edit']) {
                        foreach ($result as $key => $value) {
                            if ($key != 'id' || $key != 'skin') {
                            //if (in_array($key, $userAllowColumns)) {
                                ...
                                echo '<td><button name="edit-user-column" class="btn btn-floating z-depth-0 waves-effect blue accent-4 right"><i class="material-icons">edit</i></button></td>';
                                echo '</form>';
                                echo '</tr>';
                            }
                        }
